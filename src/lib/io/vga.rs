...
const VGA_MEM_ADDR: i32 = 0xB8000;

lazy_static! {
    /// SAFETY: 0xB8000 is always the start of the VGA buffer
...
impl VGAWriter {
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                // Don't write anything outside ascii range
    ...
    fn clear_row(&mut self, row: usize) {
        let space = ScreenChar::new(b' ', self.color_code);
